...
//
// Requirements for the `Spec` Type in `RansacEngine`
// The `Spec` type used in the `RansacEngine` template class must fulfill the following requirements:
//
// 1. ModelMatrix Type:
...
//    Spec must define a constant `MinSampleSize` that indicates the minimum number of samples required to fit the model.
